...
import { Activity, ArrowLeft, Check, ChevronRight, CircleDollarSign, FlaskConical, Gauge, History, LayoutDashboard, LockKeyhole, Play, Save, Settings2, ShieldAlert, Square, Terminal, TriangleAlert, Wifi, X } from "lucide-react";
...
const storageKey = "cashfacttory:deriv-configs:v4";
...
export function TradingDashboard() {
  ...
  const requestMode = (mode: TradingMode) => {
    ...
    if (mode === "paper") { setNotice("PAPER selecionado: simulação local ativa, sem acesso à conta Deriv."); return; }
    setNotice(mode === "demo" ? "DEMO Deriv preparado, mas bloqueado até existir uma conta demo compatível e o OAuth da Open API ser aprovado." : "REAL Deriv preparado, mas bloqueado até OAuth, validação completa em demo e desbloqueio explícito.");
  };
  ...
  return <div className="shell">
    ...
    <main>
      <header><div><div className="eyebrow">DERIV STRATEGY CONTROL</div><h1>{selected?.name ?? (view === "connection" ? "Conexão Deriv" : view === "research" ? "Pesquisa de estratégias" : "Visão geral")}</h1><p>{selected?.description ?? (view === "connection" ? "Uma única conexão segura para os quatro robôs." : view === "research" ? "Hipóteses, limitações e fontes que orientam os testes." : "Quatro estratégias rápidas, uma única plataforma e um único motor de risco.")}</p></div><div className="header-actions"><button className="platform-button" onClick={() => setView("connection")}><Wifi size={14}/><span>PLATAFORMA</span><b>Deriv</b></button></div></header>
...
function Overview({ configs, runtimes, totals, open, toggle }: { configs: BotConfig[]; runtimes: Record<BotId, BotRuntime>; totals: { balance: number; pnl: number; trades: number; winRate: number; drawdown: number }; open: (view: View) => void; toggle: (id: BotId, status: BotRuntime["status"]) => void }) {
  return <>
    ...
    <div className="section-title"><div><h2>Estratégias Deriv</h2><p>Quatro bots na mesma plataforma, com entradas de segundos a poucos minutos.</p></div><span><Wifi size={15}/>Feed paper local</span></div>
...
function Connection({ readiness, mode, requestMode }: { readiness: ConnectionReadiness; mode: TradingMode; requestMode: (mode: TradingMode) => void }) {
  const steps = [
    ...
    ["Conta demo Deriv e OAuth autorizados", readiness.oauth, "O login do site não substitui a autorização da Open API."],
  ...
  return <div className="connection-layout"><section className="connection-hero"><div className="connection-symbol"><Wifi size={28}/></div><div><span>PLATAFORMA ÚNICA</span><h2>Deriv</h2><p>Os quatro robôs usam a infraestrutura cTrader Open API para autenticação, dados, ordens, reconciliação e limites.</p></div><div className="submitted">APROVAÇÃO PENDENTE</div></section><section className="mode-card"><div className="card-heading"><div><span>AMBIENTE DE EXECUÇÃO</span><h2>Escolha onde testar</h2></div><ShieldAlert/></div><div className="mode-options">{(["paper", "demo", "live"] as TradingMode[]).map((item) => <button key={item} className={mode === item ? "selected" : ""} onClick={() => requestMode(item)}><span>{item !== "paper" && <LockKeyhole size={13}/>} {modeLabels[item]}</span><small>{item === "paper" ? "Disponível agora; dados sintéticos locais." : item === "demo" ? "Preparado; aguardando conta demo + OAuth aprovado." : "Preparado; aguardando demo validado + liberação explícita."}</small></button>)}</div></section><section className="readiness-card"><div className="card-heading"><div><span>TRAVAS DE SEGURANÇA</span><h2>Caminho até o modo real</h2></div><LockKeyhole/></div><div className="steps">{steps.map(([label, ready, note], index) => <div className={ready ? "ready" : "pending"} key={label}><span>{ready ? <Check size={15}/> : index + 1}</span><div><b>{label}</b><small>{note}</small></div></div>)}</div></section><section className="security-copy"><ShieldAlert/><div><b>O navegador não é o conector do bot</b><p>A sessão autenticada na Deriv permite uso manual. Client Secret e tokens da Open API ficam somente no servidor protegido; nenhuma ordem real é enviada por esta página enquanto as travas estiverem pendentes.</p></div></section></div>;
}
