...
const initialRows = (): Row[] => markets.map(([symbol], index) => ({ id: index + 1, symbol, parity: index % 2 ? "odd" : "even", level: 0, pnl: 0, trades: 0, wins: 0, last: "Aguardando" }));
...
export const nextMartingaleLevel = (level: number, won: boolean) => won ? 0 : level >= 2 ? 0 : level + 1;
...
export function ParityLab({ mode, trade }: { mode: TradingMode; trade: (symbol: string, parity: Parity, amount: number) => Promise<number> }) {
  ...
  const cycle = async (seed: Row) => {
    ...
    while (active.current) {
      const stake = Math.min(baseStake * (2 ** current.level), baseStake * 4);
      try {
        let profit: number;
        if (mode === "paper") {
        ...
        const won = profit > 0;
        const nextLevel = nextMartingaleLevel(current.level, won);
        current = { ...current, parity: nextParity(current.parity), level: nextLevel, pnl: current.pnl + profit, trades: current.trades + 1, wins: current.wins + (won ? 1 : 0), last: `${won ? "Ganho" : "Perda"} ${money(profit)} · próximo ${nextLevel}/2` };
  ...
  const start = () => {
    ...
    active.current = true; setRunning(true); setNotice(`${mode.toUpperCase()} ativo: cinco ciclos independentes, intervalo de 1 segundo e martingale máximo 2.`);
